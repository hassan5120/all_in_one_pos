...
$(document).ready( function(){
    ...
    var base_url="{{url('/')}}"
				$('#city').change(function() {
					...
					let req_url=base_url+"/area/"+city_id;
					$.ajax({
						headers: {
            'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
						...
						console.log(data);
						let option="<option value=''>Select Area</option>";
						data.forEach(function(item, index){
							option+="<option value="+item.id+">"+item.area+"</option>";
						});
    ...
    $(document).on('change', '#sell_list_filter_location_id, #sell_list_filter_customer_id, #sell_list_filter_payment_status, #created_by, #sales_cmsn_agnt, #service_staffs, #shipping_status, #sell_list_filter_source ,#city ,#area_select',  function() {
        sell_table.ajax.reload();
    });
